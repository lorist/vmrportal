...
from wtforms import StringField, BooleanField, PasswordField, SelectField, RadioField, IntegerField, ValidationError, validators
...
class VMRForm(Form):
    ...
    enable_overlay_text = BooleanField('Enable Overlay Text', description='Show Speaker names', validators=[])
...
class SearchForm(Form):
    def searchLength(form, field):
        if field.data and not form.allow_guests.data:
            raise ValidationError("You must allow guests if guest pin is set.")

    search = StringField('search', [DataRequired(), validators.Length(min=3, max=35)])
