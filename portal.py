...
from forms import LoginForm, VMRForm, MyDevices, ChangeDevice, SearchForm
import io
import PIL
...
def before_request():
    ...
    if 'logged_in' not in session and '/static/' not in request.path and request.endpoint != 'logout':
        ...
        if 'user_id' in session:
            ...
            g.vmrs, g.vmr_meta = getVMRs(g.email[0])
            application.logger.info('Permanent VMR count: %s', g.vmr_meta['total_count'])
            g.scheduled_conferences, g.scheduled_conference_count = totalScheduled(g.email[0])
            application.logger.info('Scheduled VMR count: %s', g.scheduled_conference_count)
            g.my_devices, g.my_devices_total = getDevices(g.email[0])
            application.logger.info('Username: %s logged in. Filter email: %s. Permanent VMR Count: %s. Scheduled VMR count: %s. Device Count: %s. ',
                g.username, g.email[0], g.vmr_meta['total_count'], g.scheduled_conference_count, g.my_devices_total)
...
def totalScheduled(email):
    total_scheduled = []
    requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
    url = "https://%s/api/admin/configuration/v1/conference/?primary_owner_email_address=%s&scheduled_conferences_count__gt=0" % (mgr_address, email)
    try:
        ...
        scheduled_conferences = json.loads(response.text)['objects']
        scheduled_conference_count = json.loads(response.text)['meta']['total_count']

    except Exception as e:
            return render_template("500.html", error = str(e))
    if scheduled_conference_count == [0]:
        error = 'error'
        application.logger.info('No VMR found for user with email: %s', email)
        return error, scheduled_conference_count

    else:

        return scheduled_conferences, scheduled_conference_count

def getVMRs(email):
    requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
    url = "https://%s/api/admin/configuration/v1/conference/?primary_owner_email_address=%s&scheduled_conferences_count=0" % (mgr_address, email)
    try:
        response = requests.get(
            url,
            auth=(mgr_user, mgr_password),
        ...
        vmr_meta = json.loads(response.text)['meta']
        application.logger.info('OFFSET: %s', vmr_meta['next'])
    except Exception as e:
            return render_template("500.html", error = str(e))
    if vmrs == []:
        error = 'error'
        application.logger.info('No VMR found for user with email: %s', email)
        return error, vmr_meta
    else:
        return vmrs, vmr_meta

# def getVMRnextprev(np):
#     requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
#     url = "https://%s%s" % (mgr_address, np)
#     try:
#         response = requests.get(
#             url,
#             auth=(mgr_user, mgr_password),
...
def getDevices(email): #my devices
    ...
    try:
        ...
        device_meta = json.loads(response.text)['meta']
        application.logger.info('getDevices result:  %s', device_meta['total_count'])
        return devices, device_meta

    except requests.exceptions.ConnectionError as e:
        return render_template('/error.html', error={'msgs': ['Connection to server failed.', 'Is it a valid domain?']})

def findVMRS(string): #my devices
    requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
    url = "https://%s/api/admin/configuration/v1/conference/?order_by=name&name__contains=%s" % (mgr_address, string)
    try:
        response = requests.get(
            url,
            auth=(mgr_user, mgr_password),
        ...
        vmr_results = json.loads(response.text)['objects']
        vmr_results_meta = json.loads(response.text)['meta']
        application.logger.info('Number search results found: %s', vmr_results_meta['total_count'])
        return vmr_results, vmr_results_meta
...
def test_vmrowenership(id):

    application.logger.info('TEST SCHEDULED: (%s)', g.scheduled_conferences)
    try:
...
def test_schedowenership(id):
    application.logger.info('TEST SCHEDULED: (%s)', g.scheduled_conferences)
    try:
        vmr = (item for item in g.scheduled_conferences if item["id"] == id).next()
        application.logger.info('TEST SCHEDULED: (%s)', vmr)
        return vmr
...
def edit(id):
    ...
    if form.validate_on_submit():
        ...
        vmr['enable_overlay_text'] = form.enable_overlay_text.data
        ...
        enable_overlay_text = vmr['enable_overlay_text']

        changeVMR(conf_url=conf_url, pin=pin, guest_pin=guest_pin, host_view=host_view,
            allow_guests=allow_guests, guests_can_present=guests_can_present, enable_overlay_text=enable_overlay_text)
        print "changing VMR"
        ...
        form.enable_overlay_text.data = vmr['enable_overlay_text']
        flash_errors(form)
    return render_template('edit.html', form=form)


@application.route('/portal/editsched/<int:id>', methods=['GET', 'POST'])
@ldap.login_required
def editsched(id):
    vmr = test_schedowenership(id)
    if vmr is 'error':
        return render_template('/error.html', error={'msgs': ['Not your conference.', 'Or, you must have got here by accident']})
    application.logger.debug('edit vmr results: (%s)', vmr)
    form = VMRForm()
...
@application.route('/portal/search', methods=['GET', 'POST'])
@ldap.login_required
def search(vmr_results=None, vmr_results_meta=None):
    if request.args.get('search', None):
        application.logger.info('search query: (%s)', request.args['search'])
        vmr_results, vmr_results_meta = findVMRS(request.args['search'])
    return render_template('search.html', vmr_results=vmr_results, vmr_results_meta=vmr_results_meta)
# def search():
#     form = SearchForm(request.form)
#     if request.method == 'GET' and form.validate_on_submit():
#         application.logger.info('search query: (%s)', form.search.data)
#         vmr_results, vmr_results_meta = findVMRS(form.search.data)
#         return render_template('search.html', vmr_results=vmr_results, vmr_results_meta=vmr_results_meta)
#     return render_template('search.html', vmr_results=vmr_results, vmr_results_meta=vmr_results_meta)
...
def user():
    ...
    vmr_config = g.vmrs
    vmr_count = g.vmr_meta['total_count']
    vmr_meta =g.vmr_meta
    application.logger.debug('config results: (%s)', vmr_config)
    # devices = g.my_devices
    # scheduled_count = g.scheduled_conference_count
    # scheduled_conferences = g.scheduled_conferences
    # device_count = g.my_devices_total
    thumbnailstring = g.thumbnailstring
    return render_template('vmrs.html',
                           user=user, vmr_config=vmr_config, vmr_count=vmr_count, vmr_meta=vmr_meta, thumbnailstring=thumbnailstring)

@application.route('/portal/scheduled')
@ldap.login_required
def scheduled():
    user = g.username
    if user == None:
    ...
    return render_template('scheduled.html',
                           user=user, scheduled_conferences=scheduled_conferences, scheduled_count=scheduled_count, thumbnailstring=thumbnailstring)
@application.route('/portal/devices')
@ldap.login_required
def devices():
    email = g.email[0]
    user = g.username
    if user == None:
        flash('User %s not found.' % accnt)
        return redirect(url_for('index'))
    devices, device_meta = getDevices(email)
    application.logger.info("Devices: %s", devices)
    thumbnailstring = g.thumbnailstring
    return render_template('devices.html',
                           user=user, devices=devices, device_meta=device_meta, thumbnailstring=thumbnailstring)
